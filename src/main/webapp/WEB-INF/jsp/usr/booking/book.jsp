	...
	<div class="con-3 flex flex-row mx-auto px-3">
			...
			<div class="guestInfor">
				...
				<c:if test="${rq.isLogined() }">
					<div class="m_point-box -mt-8 flex justify-between items-center">
						...
						<div>
							<input type="text" id="m_point" name="m_point"
								value="<fmt:formatNumber value="${rq.getLoginedMember().getM_point() }" pattern="#,###" />"
								class="text-green-500 text-2xl" style="text-align: right; width: 100px;" readonly />
							<span>원</span>
						</div>
					</div>
					<div class="flex justify-between items-center mt-6">
						...
						<div class="usePointAmount-box flex items-center">
							...
							<button type="button" id="pointDelete-btn" onclick="pointDelete();" class="mr-2">
					...
					<div class="alwaysUseAllAmount-box flex justify-end mt-1">
						<button type="button" class="text-green-500" onclick="alwaysUseAllAmount();">
							<i id="i-alwaysUseAllAmount-btn" class="fa-regular fa-circle-check"></i>
						</button>
		...
		<div class="right right_side_bar ml-14">
			...
			<div class="payment">
				...
				<c:if test="${rq.isLogined() }">
					<div class="showUsePoint-box flex justify-between items-center mb-2">
						...
						<div class="text-lg">
							...
							<input id="showUsePoint" type="text" style="text-align: right; width: 70px;" readonly />
							원
						</div>
					</div>
					<div class="realAmount-box flex justify-between items-center mb-2">
						<div class="font-extrabold">실제 결제 금액</div>
						<div class="text-xl font-extrabold text-red-600">
							<input id="realAmount" type="text" name="realAmount" value="" style="text-align: right; width: 90px;" readonly />원
						</div>
					</div>
				</c:if>
				<ul class="mt-5">
				...
				<button class="text-center btn btn-active btn-secondary w-full mt-4 mb-10" onclick="writeBook();">결제하기</button>
			</div>
		</div>
	</div>
</section>
		 ...
		 $(document).ready(function() {
	        $('#usepointAmount').keyup(function(e) {
	          ...
	          if(intUsePointAmount != 0){
	        	  ...
	        	  var m_value = parseInt($('#m_point').val().replace(/,/g,""));
	        	  var p_value = parseInt($('#usepointAmount').val().replace(/,/g,""));
...
// doBook에 보내기전에 입력 내용 체크하고, localStorage에 customerName 저장한 후 doBook 실행 하고 결제창 띄우는 함수 시작
	function writeBook(){
	    ...
	    let $orderId = $('#orderId').val();
 		...
 		let $countOfAdult = $('#countOfAdult').val();
 		let $countOfChild = $('#countOfChild').val();
 		let $DateAndDayOfTheWeekOfChkin = $('#DateAndDayOfTheWeekOfChkin').val();
 		let $DateAndDayOfTheWeekOfChkout = $('#DateAndDayOfTheWeekOfChkout').val();
 		let $amount = $('#amount').val();
 		let $pay_point = $('#usepointAmount').val();
	    ...
	    $.get('../booking/doBook', {
	    	...
	    	DateAndDayOfTheWeekOfChkin : $DateAndDayOfTheWeekOfChkin,
	    	DateAndDayOfTheWeekOfChkout : $DateAndDayOfTheWeekOfChkout,
	    	amount : $amount,
	    	pay_point : $pay_point,
			...
			if($pay_point == null){
				$pay_point = parseInt(0);
			} else {
				$pay_point = parseInt($pay_point);
			}
			$realAmount = parseInt($realAmount);
			
			if($customerName == '' || $customerName == null || $cellphoneNo == '' || $cellphoneNo == null){
				return false;
...
// doBook에 보내기전에 입력 내용 체크하고, localStorage에 customerName 저장한 후 doBook 실행 하고 결제창 띄우는 함수 끝
